...
export const renderP = () => {
  const productsContent = document.querySelector('#products .products__content')
  ...
  products.forEach(product => {
    html += `
      ...
      <div class="products__data">
        <h2 class="products__price">${price(product.price)} <span class="products__quantity">| Stock: ${product.quantity}</span></h2>
  ...
  productsButton.forEach(button => {
    button.addEventListener('click', () => {
      ...
      if (product && product.quantity > 0) {
        ...
        window.alert('Lo sentimos, no tenemos mas mercancia')
      }
    })
  })
